...
import { ArrowLeft, ChevronDown, MapPin, Star, Wifi, Bed, Bath, Maximize, Edit, Trash2 } from "lucide-react"
...
export default function ItemPage({ params }: { params: { id: number } }) {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const response = await getReviewWithHotelID(session.user.token, hotel.id);
  ...
  const handleEditReview = (reviewId: string, currentComment: string, currentRating: number) => {
    setEditMode(true);
              ...
              <div className="mb-4">
                ...
                {editMode && (
                  <div className="pt-[10px] pb-[30px] flex flex-col border p-4 rounded-lg bg-gray-50">
                    <h3 className="font-medium mb-2">Edit Your Review</h3>
                    <div className="flex items-center gap-3 mb-[2px]">
                      <div className="w-8 h-8 bg-gray-200 rounded-full"></div>
                      <div className="flex flex-col">
                        <span className="text-gray-500 text-sm">{session?.user.name}</span>
                      ...
                      {session?.user._id === review.user?._id && (
                        <div className="flex gap-2">
                          <button 
                            ...
                            onClick={() => handleEditReview(review._id, review.comment, review.rating)}
                          >
